        ...
        for c1 in range(self.num_clusters):
            ...
            for c2 in range(c1 + 1, self.num_clusters):
                ...
                while edges_added < num_edges and attempts < max_attempts:
                    ...
                    if (u, v) not in used_pairs:
                        used_pairs.add((u, v))
                        w = random.randint(int((self.weight_range[0] + self.weight_range[1]) / 2), self.weight_range[1])
                        graph.add_edge(u, v, w)
                        edges_added += 1
                    attempts += 1
            ...
            for c2 in range(c1 + 1, self.num_clusters):
                ...
                while edges_added < expected:
                    ...
                    if (u, v) not in used:
                        used.add((u, v))
                        if random.random() < prob:
                            w = random.randint(self.weight_range[0], int((self.weight_range[0] + self.weight_range[1]) / 2))
                            graph.add_edge(u, v, w)
                            edges_added += 1
